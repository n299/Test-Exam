...
class UserExamsController < ApplicationController
  ...
  def create
    user_exam = UserExam.create_user_exam(params[:exam_id], current_user)
  ...
  def exam_start
    ...
    @exam = @exam_start.decorate.exam_user
  end

  def update
    score = total_exam_score
    user_exam = UserExam.find_by(id: params[:id])
    params[:list_answer] = [] if params[:list_answer].blank?
  ...
  def show
    ...
    return redirect_to(exams_url, alert: 'This exam not exit!') unless @user_exam.check_user_exam(current_user)
  end

  def index
    @user_exams = UserExam.list_user_exam(current_user).decorate
  end
